...
import { executiveCards } from "@/lib/executive-cards";

const CATEGORIES = ["All", "Polished Speaking", "Money Deflection", "Home Energy", "Comeback", "Age Advantage", "Physical Confidence", "Indian Business", "Recovery", "Small Talk", "Long Game", "Gratitude", "Boardroom", "Language", "Strategy", "Corporate Body", "Executive Dress", "Jargon", "Negotiation", "Leadership", "Communication", "Finance", "Personal Brand", "Meetings", "Client Mgmt", "Tech Executive", "EQ", "Productivity", "Executive Wisdom"];
...
export default function ExecutivePage() {
  ...
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem("mmam-fav-exec") || "[]");
  ...
  const toggleFav = (id: number) => {
    ...
    setFavs(next);
    localStorage.setItem("mmam-fav-exec", JSON.stringify([...next]));
  };

  let cards = executiveCards;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-10">
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight">Executive MBA</h1>
        <p className="mt-2 text-muted">
          {executiveCards.length} tips to transform from developer to corporate executive. Boardroom confidence, management jargon, negotiation, and presence.
        </p>
      </div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setShuffled(!shuffled)} className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${shuffled ? "bg-accent text-white" : "bg-surface text-muted"}`}>
          🔀 Shuffle
        </button>
      </div>

      <div className="mb-4">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tips..." className="w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm placeholder:text-muted focus:border-accent focus:outline-none" />
      </div>
        ...
        <div className="space-y-3">
          {cards.map((card) => (
            <div key={card.id} className={`group relative rounded-xl border p-5 transition-all ${favs.has(card.id) ? "border-accent/30 bg-accent/5" : "border-border bg-surface"}`}>
              ...
              {card.author && <p className="mt-2 text-xs font-medium text-accent">— {card.author}</p>}
            </div>
          ))}
        </div>
      )}
